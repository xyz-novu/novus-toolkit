...
export interface FormData {
  ...
  researchApproach?: "interviews" | "desk" | "both";
...
function buildProjectContext(data: FormData): string {
...
function buildMOC(data: FormData): string {
  ...
  if (data.stakeholders.length > 0 && data.stakeholders.some((s) => s.role)) {
    foundationLinks.push(
      "- [stakeholder-map](../foundations/stakeholders/stakeholder-map.md) — influence/alignment matrix"
    );
  }
  ...
  for (const [key, slug] of Object.entries(stubMap)) {
    if (data.foundations[key as keyof typeof data.foundations]) {
      foundationLinks.push(
        `- [${slug}](../foundations/${slug}.md)`
      );
    }
  }
  if (foundationLinks.length > 0) {
    result = result.replace(
      /<!-- When foundation files are seeded[\s\S]*?-->/,
  ...
  if (data.hasResearch) {
    result = result.replace(
      /<!-- When the research plan is seeded[\s\S]*?-->/,
...
function buildDecisionLog(data: FormData): string {
...
function buildStakeholderMap(data: FormData): string | null {
  const hasRoles = data.stakeholders.some((s) => s.role && s.role !== "TBD");
  if (data.stakeholders.length === 0 || !hasRoles) return null;

  // Build stakeholder rows for the table
  const rows = data.stakeholders
    .map(
      (s) =>
        `| ${s.name} | ${s.role} | TBD | TBD | ${s.domain || "TBD"} |`
    )
    .join("\n");

  // Build domain groupings
  const domains = new Map<string, Stakeholder[]>();
...
function buildResearchPlan(data: FormData): string | null {
  if (!data.hasResearch) return null;
...
const STUB_CONFIGS: Record<string, StubConfig> = {
  brief: {
    slug: "brief",
    title: "Brief",
    description: "Client brief, RFP, or original project ask.",
    guidance:
      "Paste or summarise the client brief / RFP here. Include objectives, constraints, and any stated success criteria.",
  },
  designSystem: {
    slug: "design-system",
    title: "Design System",
    ...
    guidance:
      "Document the design system, brand guidelines, colour palette, typography, and component conventions. Link to external resources (Figma, Storybook) where applicable.",
  },
  constraints: {
    slug: "constraints",
    title: "Constraints",
    ...
    guidance:
      "List known constraints: platforms, tooling, technical limitations, compliance requirements, team capacity. Use a table for structured constraints.",
  },
  businessKpis: {
    ...
    guidance:
      "Document target KPIs, success metrics, commercial model, and any business context that should inform design and strategy decisions.",
  },
};

function buildFoundationStub(type: string, data: FormData): string {
  const config = STUB_CONFIGS[type];
  if (!config) throw new Error(`Unknown foundation stub type: ${type}`);
...
export function scaffold(data: FormData): ScaffoldResult {
  ...
  const foundationKeys: (keyof typeof data.foundations)[] = [
    "brief",
    "designSystem",
    "constraints",
    "businessKpis",
