...
import type { FormData } from "./scaffold-engine.js";
...
function validateFormData(body: unknown): { valid: true; data: FormData } | { valid: false; error: string } {
  ...
  if (!b.projectType || !["client", "internal"].includes(b.projectType as string)) {
  ...
  const rawOutputDir = b.outputDir as string;
  ...
  return {
    valid: true,
    data: {
      projectType: b.projectType as "client" | "internal",
      clientName: b.clientName as string | undefined,
      projectName: b.projectName as string,
      projectTitle: b.projectTitle as string,
      outputDir: resolvedOutputDir,
      goal: (b.goal as string) ?? "",
      scope: (b.scope as string) ?? "",
      timeline: (b.timeline as string) ?? "",
      strategicContext: (b.strategicContext as string) ?? "",
      stakeholders: Array.isArray(b.stakeholders) ? b.stakeholders as FormData["stakeholders"] : [],
      foundations: {
        brief: !!(b.foundations as Record<string, boolean>)?.brief,
        designSystem: !!(b.foundations as Record<string, boolean>)?.designSystem,
        constraints: !!(b.foundations as Record<string, boolean>)?.constraints,
        businessKpis: !!(b.foundations as Record<string, boolean>)?.businessKpis,
      },
      hasResearch: !!(b.hasResearch),
      primaryQuestion: b.primaryQuestion as string | undefined,
      researchApproach: b.researchApproach as FormData["researchApproach"],
      interviewCount: b.interviewCount as string | undefined,
    },
  };
}
